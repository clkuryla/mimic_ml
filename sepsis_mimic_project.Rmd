...
predicted_mortality <- ifelse(saps2_score > threshold, 1, 0)  # 1 = Predicted to die, 0 = Predicted to survive

confusion_matrix <- table(Predicted = data$predicted_mortality, Actual = data$hospital_expire_flag)

print("Confusion Matrix:")
print(confusion_matrix)

true_positive <- confusion_matrix[2, 2]  # Predicted to die and actually died
false_positive <- confusion_matrix[2, 1]  # Predicted to die but survived
true_negative <- confusion_matrix[1, 1]  # Predicted to survive and actually survived
false_negative <- confusion_matrix[1, 2]  # Predicted to survive but died
...
thirtyday_expire_flag <- as.numeric(data$thirtyday_expire_flag)

roc_curve <- roc(thirtyday_expire_flag, saps2_score)

plot(roc_curve, col = "blue", main = "ROC Curve for SAPS II Predictions")
...
###############
# Some models cannot handle NAs. (Like Random Forest)
colSums(is.na(model_data))
# There are not many, compared to the size of the dataset, so we will just remove them

train_data_na_omit <- model_data[train_index, ] %>% na.omit()
test_data_na_omit  <- model_data[-train_index, ] %>% na.omit()

# Separate features and labels
x_train_na_omit <- train_data %>% na.omit() %>% select(-thirtyday_expire_flag)
y_train_na_omit <- train_data %>% na.omit() %>% select(thirtyday_expire_flag)
#train_y <- train_data$thirtyday_expire_flag

x_test_na_omit <- test_data %>% na.omit() %>% select(-thirtyday_expire_flag)
y_test_na_omit <- test_data %>% na.omit() %>% select(thirtyday_expire_flag)
...
# We will now analyze the data using various tree-based methods

library(tree)

tree_sepsis <- tree(thirtyday_expire_flag ~ . , train_data)
summary(tree_sepsis)
plot(tree_sepsis)
text(tree_sepsis, pretty = 0)


tree_cv <- cv.tree(tree_sepsis)
tree_cv
plot ( tree_cv$size , tree_cv$dev , type = "b")
plot ( tree_cv$k , tree_cv$dev , type = "b")

tree_cv <- cv.tree(tree_sepsis, FUN = prune.misclass)
tree_cv

plot ( tree_cv$size , tree_cv$dev , type = "b")
plot ( tree_cv$k , tree_cv$dev , type = "b")

tree_pruned <- prune.misclass(tree_sepsis, best = 6)
plot(tree_pruned)
text(tree_pruned, pretty = 0)

model_tree_pruned <- prune.misclass(tree_sepsis, best = 6)
pred_tree_pruned <- predict(tree_pruned_model, test_data, type = "class")

# For fun, fit on all data:
tree_sepsis_full <- tree(thirtyday_expire_flag ~ . , model_data)
summary(tree_sepsis_full)
plot(tree_sepsis_full)
text(tree_sepsis_full, pretty = 0)

# Those variables are familiar!!
...
# Bagging
set.seed(1)
bag_sepsis <- randomForest(thirtyday_expire_flag ~ ., 
                           data = train_data,
                           )
...
library(randomForest)
set.seed(1248)
model_rf_baseline <- randomForest(thirtyday_expire_flag ~ ., data = train_data, importance = TRUE)
#saveRDS(model_rf_baseline, "models_etc/model_rf_baseline.rds")


# Evaluate
pred_rf_baseline <- predict(rf_baseline, test_data)
# saveRDS(pred_rf_baseline, "models_etc/pred_rf_baseline.rds")

library(caret)
cm <- confusionMatrix(predictions, test_data %>% pull(thirtyday_expire_flag))
cm 

# write.csv(data.frame(predictions = predictions), "predictions.csv", row.names = FALSE)
# saveRDS(cm, "confusion_matrix.rds")
# write.csv(importance(rf_final), "feature_importance.csv", row.names = TRUE)

# Tune parameters manually 

# cm$overall['Accuracy']
# cm$overall['Sensitivity']
# 
# # Tuning
# 
# # mtry
# mtry_values <- seq(2, sqrt(ncol(train) - 1), by = 1) 
# mtry_results <- data.frame(mtry = mtry_values, 
#                       Accuracy = NA,
#                       Sensitivity = NA,
#                       Specificity = NA,
#                       PPV = NA,
...
#   set.seed(1248)
#   rf <- randomForest(thirtyday_expire_flag ~ ., data = train_data, mtry = mtry_values[i])
#   predictions <- predict(rf, test_data)
#   cm <- confusionMatrix(predictions, test$target)
#   mtry_results$Accuracy[i] <- cm$overall['Accuracy']
#   mtry_results$Sensitivity[i] <- cm$byClass['Sensitivity']
#   mtry_results$Specificity[i] <- cm$byClass['Specificity']
#   mtry_results$PPV[i] <- cm$byClass['Pos Pred Value']
#   mtry_results$NPV[i] <- cm$byClass['Neg Pred Value']
#   mtry_results$Prevalence[i] <- cm$byClass['Prevalence']
#   mtry_results$Detection_Rate[i] <- cm$byClass['Detection Rate']
#   mtry_results$Detection_Prevalence[i] <- cm$byClass['Detection Prevalence']
#   mtry_results$Balanced_Accuracy[i] <- cm$byClass['Balanced Accuracy']
#   mtry_results$Precision[i] <- cm$byClass['Precision']
#   mtry_results$Recall[i] <- cm$byClass['Recall']
# }
# 
# library(reshape2)
# mtry_results_melted <- melt(mtry_results)
# 
# ggplot(mtry_results_melted, aes(x = mtry, y = Accuracy)) +
#   geom_line() +
#   geom_point() +
#   labs(title = "Tuning mtry", x = "Number of Variables (mtry)", y = "Accuracy")
# 
# 
# # do the same for ntree and nodesize (minimmum size of terminal nodes)
# 
...
# )
# 
# predictions_final <- predict(rf_final, test)
# confusionMatrix(predictions_final, test$target)
# 
# varImpPlot(rf_final)

# Example: Store results for combined tuning of mtry and ntree
tuning_results <- expand.grid(mtry = 1:(ncol(train) - 1), ntree = seq(50, 500, by = 50))
tuning_results$Accuracy <- NA

# Loop through combinations
for (i in 1:nrow(tuning_results)) {
  rf_tuned <- randomForest(target ~ ., data = train, 
                           mtry = tuning_results$mtry[i],
                           ntree = tuning_results$ntree[i])
  pred <- predict(rf_tuned, test)
  cm <- confusionMatrix(pred, test$target)
  tuning_results$Accuracy[i] <- cm$overall["Accuracy"]
}

# Plot Accuracy vs. mtry, grouped by ntree
ggplot(tuning_results, aes(x = mtry, y = Accuracy, color = as.factor(ntree))) +
  geom_line() +
  geom_point() +
  labs(title = "Tuning mtry and ntree for Random Forest",
       x = "mtry",
       y = "Accuracy",
...
# Extract feature importance
feature_importance <- importance(rf_final)
...
tuneGrid <- expand.grid(mtry = c(2, 4, 6)) # Adjust mtry based on number of features
...
model_rf_grid <- rf_tuned

varImpPlot(model_rf_grid$finalModel)

#saveRDS(model_rf_grid, "models_etc/model_rf_grid.rds")
pred_rf_grid <- predict(model_rf_grid, test_data)
#saveRDS(pred_rf_grid, "models_etc/pred_rf_grid.rds")

a <- readRDS("models_etc/model_rf_grid.rds")
class(a) <- "randomForest"
varImpPlot(a)
...
So my final variables for moving forward predictions are:
  
  model_tree_pruned
  pred_tree_pruned
  
  model_rf_baseline <- readRDS("models_etc/model_rf_baseline.rds")
  pred_rf_baseline <- readRDS("models_etc/pred_rf_baseline.rds")
  
  # I am going to do this tomorrow. Can just use this one for now.
  model_rf_optimal_param <- readRDS("models_etc/model_rf_baseline.rds")
  pred_rf_optimal_param <- readRDS("models_etc/pred_rf_baseline.rds")
  
  model_rf_grid <- readRDS("models_etc/model_rf_grid.rds")
  pred_rf_grid <- readRDS("models_etc/pred_rf_grid.rds")


I think caret is a good start:
  
library(caret)
confusionMatrix(predictions, test_data %>% pull(thirtyday_expire_flag))

We also should do c.v. and pick metrics, etc
